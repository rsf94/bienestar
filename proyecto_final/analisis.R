...
data <- file.path("C:/Users/rsf94/Google Drive/MAESTRÍA ITAM/2do semestre/Bienestar y política social/Bienestar_equipo/trabajo_final/data_raw")
...
strsplit(data_ind$ID_PERS[1]," ")[1]

data_ind <- data_ind %>% separate(ID_PERS,c("id_hogar","id_ind")," ")

# elimino 19 registros donde no contamos con el ID del individuo
data_ind <- data_ind %>% select(id_ind) %>% filter(!is.na(id_ind))

check_nas(data_ind) %>% filter(pct_miss<60)

length(data_ind)
nrow(check_nas(data_ind) %>% filter(pct_miss<60))
